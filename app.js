...
// --- PENERAPAN RATE LIMIT GLOBAL ---
bot.use(checkRateLimit);
const userRateLimit = {}; 
const RATE_LIMIT_MS = 1500; // Batas 1 pesan per 1,5 detik (1500 milidetik)
const RATE_LIMIT_MESSAGE = '⚠️ *Sabar, sayang!* Jangan terlalu cepat atau spam. Aku capek tau, pelan pelan aja ya?';

logger.info('User rate limit initialized');
...
function checkRateLimit(ctx, next) {
    const userId = ctx.from.id;
    const now = Date.now();
    const lastMessageTime = userRateLimit[userId] || 0;
    ...
    if (now - lastMessageTime < RATE_LIMIT_MS) {
        ...
        if (isCallback) {
            ...
            ctx.answerCbQuery(RATE_LIMIT_MESSAGE.replace(/\*/g, '').replace('Coba lagi dalam 1 detik.', ''), true).catch(e => {
                // Log error jika answerCbQuery gagal
                logger.warn(`Failed to answerCbQuery for rate limit for user ${userId}: ${e.message}`);
            });
        } else {
            // Untuk pesan teks atau perintah, kirim balasan
            ctx.reply(RATE_LIMIT_MESSAGE, { parse_mode: 'Markdown' }).catch(e => {
                // Log error jika reply gagal
                logger.warn(`Failed to reply for rate limit for user ${userId}: ${e.message}`);
            });
        }
...
bot.command(['start', 'menu'], async (ctx) => {
  ...
  if (!isJoined) {
    ...
    const message = 
      ...
      `Untuk menggunakan bot ini, Anda wajib bergabung ke channel resmi kami.\n\n` +
...
bot.action('check_force_subscribe', async (ctx) => {
    await ctx.answerCbQuery();
    const userId = ctx.from.id;
    const isJoined = await isUserJoinedChannel(userId); // Menggunakan fungsi yang baru

    if (isJoined) {
        // Cek apakah pesan masih bisa diedit.
        try {
        ...
        const message = 
          ...
          `Mohon bergabung ke channel ini terlebih dahulu: [${CHANNEL_USERNAME}](${channelLink}).\n\n` +
...
bot.on('text', async (ctx) => {
  ...
  if (state.step.startsWith('username_') && state.action !== 'trial') {
    ...
    db.get('SELECT quota, iplimit, harga, nama_server FROM Server WHERE id = ?', [state.serverId], async (err, server) => {
      ...
      if (action === 'create') {
          if (type === 'vmess') { msg = await createvmess(username, exp, quota, iplimit, serverId); } else if (type === 'vless') { msg = await createvless(username, exp, quota, iplimit, serverId); } else if (type === 'trojan') { msg = await createtrojan(username, exp, quota, iplimit, serverId); } else if (type === 'shadowsocks') { msg = await createshadowshocks(username, exp, quota, iplimit, serverId); } else if (type === 'ssh') { msg = await createssh(username, password, exp, iplimit, serverId); }
